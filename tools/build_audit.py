...
BLOCKED_REASONS = {
    ...
    "AIX": "no RUNNER-signed AIX capture manifest for the current revision",
...
BUTTON_DOC = BLOB + "documentation/2-components/button.en-US.md"
BUTTON_SAMPLE = BLOB + "samples/capabilities/pages/close/index.ink"
...
POLICY = {
    ...
    "focus.element": ("CAP-ELEMENT-FOCUS", SSSD, None, [("DOC", "focus", FOCUS_DOC)]),
    "ui.button": ("CAP-BUTTON", ["SOURCE", "STATIC", "AIX", "STUDIO", "DEVICE"], None, [("DOC", "button", BUTTON_DOC), ("SAMPLE", "button implementation", BUTTON_SAMPLE)]),
    "event.bindtap": ("CAP-BINDTAP", ALL, None, [("DOC", "button", BUTTON_DOC), ("SAMPLE", "button implementation", BUTTON_SAMPLE)]),
    ...
    "input.scroll.unknown": ("CAP-INPUT-SCROLL", SSLSD, "Unresolved scroll input", [("SEARCH-SCOPE", "Page/open-agent-format index", OPEN_AGENT_INDEX), ("SEARCH-SCOPE", "component index", COMPONENT_INDEX)]),
    "input.fallback.unknown": ("CAP-INPUT-FALLBACK", SSLSD, "Unresolved non-sensor fallback", [("SEARCH-SCOPE", "Page/open-agent-format index", OPEN_AGENT_INDEX), ("SEARCH-SCOPE", "component index", COMPONENT_INDEX)]),
...
BEHAVIOR = {
    ...
    "focus.element": ("Each actionable element exposes focus and activation in the intended order", "Missing, trapped, or blurred element focus preserves a visible recovery path", "Repeated render and reopen do not retain stale element focus handlers"),
    "ui.button": ("The button exposes and performs its intended action", "Disabled, unavailable, or repeated activation remains explicit and recoverable", "Repeated render and reopen preserve one current button action"),
    "event.bindtap": ("One owned tap invokes the bound action exactly once", "Ignored, repeated, or unavailable tap delivery does not duplicate or dead-end the action", "Hide/show and unload do not retain stale tap handlers or work"),
    ...
    "input.scroll.unknown": ("The declared product intent affects only its owned target", "Unsupported or ignored scrolling preserves host behavior", "Repeated open does not duplicate scroll handling"),
    "input.fallback.unknown": ("The core task remains usable without the primary sensor", "Fallback failure leaves a clear exit without duplicate action", "Fallback remains available after hide/show and reopen"),
